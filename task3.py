...
def func(lst):
    ...
    for i in lst:
        if i['course'] == 'python':
            courses['python'].append(i)
        elif i['course'] == 'javascript':
            courses['javascript'].append(i)
        else:
            courses['java'].append(i)
    return courses
...
def func_3(names, names_ages):
    for i in names:
        try:
            print(f'{i} - {names_ages[i]}')
        except:
            print(f'Имени {i} не существует')
    return None
func_3(names, names_ages)
